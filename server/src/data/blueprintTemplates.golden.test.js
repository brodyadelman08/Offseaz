...
//   2. Explicit routing-regression assertions for the 3 real bugs fixed on
//      feat/blueprint-quick-wins — Football K/P, Track "Throws"/"Jumps", and
//      all 6 soccer positions — so a future regex change that reintroduces
//      any of them fails with a named, specific assertion, not just an
//      unexplained snapshot diff.
//   3. Explicit phase-rep-arc assertions for the 10 sport groups that got
//      Change 1/3/4 coverage on the same branch, so reverting that coverage
//      for any one of them fails immediately and by name.
//   4. A focused injury-substitution golden set — Football/WR (a realistic,
//      correctly-routing free-text input that's verified to exercise all 4
//      areas implemented on THIS branch) × Shoulder/Knee/Back/Hip/None.
//
// Scope note on (4): this branch has only 4 injury areas (Shoulder, Knee,
// Back, Hip — see applyInjuryAdjustments). The 9-area/flat-50% system
// (adding Quadriceps, Hamstring, Ankle, Elbow, Wrist) lives on the separate,
// still-unmerged feat/injury-system-upgrade branch (PR #16). Once that
// merges, extend KNEE_ETC below with the 5 additional areas using the exact
// same pattern — everything here is written so that's a small addition, not
// a restructure.
...
// Football/WR (survey-realistic free text, verified to correctly route to
// Skill and to exercise all 4 substitution triggers implemented on this
// branch — Back Squat/Bench Press/Trap Bar Deadlift/Bulgarian Split Squat +
// Single Leg RDL all appear in its real content) × each implemented area.
// A future change that renames one of those triggers without updating the
// matching applyXAdjustments regex will show up here as a snapshot diff
// where the substitution silently stops firing.

describe('Injury-substitution golden set — Football/WR × every area implemented on this branch', () => {
  const AREAS = ['None', 'Shoulder', 'Knee', 'Back', 'Hip']

  for (const area of AREAS) {
    test(`injury_areas=[${area}] — week 1, Days 1 & 2`, () => {
      const bp = generateBlueprintForAthlete(mkSurvey({
        sport: 'Football', position: 'WR', injury_areas: area === 'None' ? [] : [area],
      }))
  ...
  test('baseline (no injury) vs each area actually changes the text — a substitution that silently stops firing is a real regression, not just a snapshot diff', () => {
    const baseline = generateBlueprintForAthlete(mkSurvey({ sport: 'Football', position: 'WR' })).weeks[0]
    for (const area of ['Shoulder', 'Knee', 'Back', 'Hip']) {
      const withInjury = generateBlueprintForAthlete(mkSurvey({
        sport: 'Football', position: 'WR', injury_areas: [area],
      })).weeks[0]
